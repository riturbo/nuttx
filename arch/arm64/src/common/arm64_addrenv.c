/****************************************************************************
 * arch/arm64/src/armv8-a/arm64_addrenv.c
 *
...
static int up_addrenv_initdata(uintptr_t l2table)
{
  ...
  virtptr = (uint32_t *)arm64_pgvaddr(l2table);
  ...
  virtptr = (uint32_t *)arm64_pgvaddr(paddr);
                      ...
                      arch_addrenv_t *addrenv)
{
  ...
  ret = arm64_addrenv_create_region(addrenv->text, ARCH_TEXT_NSECTS,
                                  CONFIG_ARCH_TEXT_VBASE, textsize,
                                  0x400 | 0x40 |MT_NORMAL | MT_RW | PTE_PAGE_DESC);
  if (ret < 0)
    {
  ...
  ret = arm64_addrenv_create_region(addrenv->data, ARCH_DATA_NSECTS,
  ...
  ret = arm64_addrenv_create_region(addrenv->heap, ARCH_HEAP_NSECTS,
...
int up_addrenv_destroy(arch_addrenv_t *addrenv)
{
  ...
  arm64_addrenv_destroy_region(addrenv->text, ARCH_TEXT_NSECTS,
                             CONFIG_ARCH_TEXT_VBASE, false);

  /* Destroy the .bss/.data region */

  arm64_addrenv_destroy_region(addrenv->data, ARCH_DATA_NSECTS,
                             CONFIG_ARCH_DATA_VBASE, false);

#ifdef CONFIG_BUILD_KERNEL
  /* Destroy the heap region */

  arm64_addrenv_destroy_region(addrenv->heap, ARCH_HEAP_NSECTS,
  ...
  arm64_addrenv_destroy_region(addrenv->heap, ARCH_SHM_NSECTS,
...
int up_addrenv_select(const arch_addrenv_t *addrenv)
{
  ...
  for (vaddr = CONFIG_ARCH_TEXT_VBASE, i = 0;
       ...
       vaddr += SECTION_SIZE, i++)
    {
      ...
      if (paddr)
        {
          binfo("text: set l1 entry (paddr=%lx vaddr=%lx)\n", paddr, vaddr);
          mmu_l1_setentry(paddr, vaddr, MMU_L1_PGTABFLAGS);
        }
      else
        {
          binfo("text: clear l1 (vaddr=%lx)\n", vaddr);
  ...
  for (vaddr = CONFIG_ARCH_DATA_VBASE, i = 0;
       ...
       vaddr += SECTION_SIZE, i++)
    {
      ...
      if (paddr)
        {
          binfo("data: set l1 entry (paddr=%lx vaddr=%lx)\n", paddr, vaddr);
          mmu_l1_setentry(paddr, vaddr, MMU_L1_PGTABFLAGS);
        }
      else
        {
          binfo("data: clear l1 (vaddr=%lx)\n", vaddr);
          //mmu_l1_clrentry(vaddr);
        }
    }
  ...
  for (vaddr = CONFIG_ARCH_HEAP_VBASE, i = 0;
       ...
       vaddr += SECTION_SIZE, i++)
    {
      ...
      if (paddr)
        {
          binfo("heap: set l1 entry (paddr=%lx vaddr=%lx)\n", paddr, vaddr);
          mmu_l1_setentry(paddr, vaddr, MMU_L1_PGTABFLAGS);
        }
      else
        {
