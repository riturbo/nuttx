                              ...
                              uint32_t mmuflags)
{
  ...
  binfo("listlen=%lu vaddr=%08lx regionsize=%ld, mmuflags=%08x\n",
  ...
  if (npages > ARCH_SECT2PG(listlen))
    {
      berr("ERROR: npages=%u listlen=%lu\n", npages, listlen);
      return -E2BIG;
    }
  ...
  for (i = 0; i < npages; i += ENTRIES_PER_L2TABLE)
    {
      ...
      paddr = mm_pgalloc(1);
      binfo("a new l2 page table (paddr=%lx)\n", paddr);
      if (!paddr)
        {
          return -ENOMEM;
        }
      ...
      l2table = (uintptr_t *)arm64_pgvaddr(paddr);
